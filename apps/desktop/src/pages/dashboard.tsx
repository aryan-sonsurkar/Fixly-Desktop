...
export function DashboardPage() {
  const { data, setData, loading, setLoading, briefing, setBriefing, briefingLoading, setBriefingLoading, widgets } = useDashboardStore();
  const { setOpen: setSearchOpen } = useSearchStore();
  const dragItem = useRef<number | null>(null);
  const [orderedWidgets, setOrderedWidgets] = useState(widgets.filter((w) => w.visible).sort((a, b) => a.order - b.order));
  ...
  const stats = data?.stats || { total: 0, completed: 0, pending: 0, in_progress: 0, overdue: 0, due_today: 0, due_this_week: 0, completion_percentage: 0 };
        ...
        <div className="space-y-2 text-sm">
          {stats.pending > 0 ? (
            [
              ...
              ...(stats.overdue > 0 ? [{ label: `${stats.overdue} overdue`, color: "text-red-500" as const }] : []),
            ].map((t) => (
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6 p-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {orderedWidgets
          .filter((w) => ["focus", "xp", "score", "pomodoro"].includes(w.type))
          .map((widget) => (
            <motion.div
              key={widget.id}
              layout
      ...
      >
        {orderedWidgets.filter((w) => w.type === "briefing").map((widget) => (
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        {orderedWidgets
          .filter((w) => ["assignments", "tasks", "actions", "emails"].includes(w.type))
          .map((widget) => (
            <motion.div
              key={widget.id}
              layout
